...
最小二乘法的理念是承认观测误差的存在，误差是围绕真值上下波动的，当平方误差最小时即为真值。具体应用时，算法的目标是要求线性方程中未知权重w和b，当经验风险最小化时的权重认为是最优的。为了简化，我们经常在推导中忽略b，因为b可以认为存在一个常数列x=1对应的权重w的一个分量，不必单独另求。

以下向量定义为列向量，注意在不同步骤定义谁是谁的函数。
...
文献里经常说梯度是让损失函数下降的方向。我们先看单个参数，就决定了下一步的大小(学习率还会拦一道)和方向(也就是正负)。对多个特征需要的多参数，主要是决定了不同参数大小的对比，也就成了多向量空间中的方向。对于多个样本来说，可以认为每个样本都会让参数"优化"移动一次。
